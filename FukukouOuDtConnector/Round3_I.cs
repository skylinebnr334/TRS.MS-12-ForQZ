...
namespace FukukouOuDtConnector
{
    ...
    {
        public int Get_NextRound_R3()
        {
            string point = this.s_url3 + "/next_round";
        ...
        public SendResult Send_R3_Result(Round3_JSONData sendData)
        {
            if (PluginHost.SendType == SendTypes.Inquire)
            {
                string point = this.s_url3 + "/status";
                HttpRequestMessage rq = new HttpRequestMessage(HttpMethod.Get, point);
                string rbodyS;
                HttpStatusCode hscode = HttpStatusCode.NotFound;
                Task<HttpResponseMessage> resp;
            ...
            {
                {
                    string point = this.s_url3 + "/set_round_data";
                ...
                }
                {
                    ...
                    string rbodyS;
                    HttpStatusCode hscode = HttpStatusCode.NotFound;
                    var nextRData = new Round3_next_roundJSONData();
