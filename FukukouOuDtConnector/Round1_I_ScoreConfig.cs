...
namespace FukukouOuDtConnector
{
    ...
    {
        public Round1_I_ScoreConfig_JSONData get_DefaultScoreConfig()
        {
            string point = this.s_url + "/get_score_setting";
            ...
            try
            {
                ...
                var getDt = JsonSerializer.Deserialize<Round1_I_ScoreConfig_JSONData[]>(rbodyS);
                foreach(var dt in getDt.Where(z=>z.index==-810))
                {
                    return dt;
                }
                return new Round1_I_ScoreConfig_JSONData()
                {
                    index = -810,
                    ask_throw = 0,
                    correct = 1,
        ...
        public SendResult SendScoreConfig(Round1_I_ScoreConfig_JSONData data)
        {
            if (PluginHost.SendType == SendTypes.Sell)
            {
                {
                    string point = this.s_url + "/set_score_setting";
