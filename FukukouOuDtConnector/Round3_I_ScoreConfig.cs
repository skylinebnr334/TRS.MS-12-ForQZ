...
namespace FukukouOuDtConnector
{
    ...
    {
        public Round3_I_ScoreConfig_JSONData get_DefaultScoreConfigR3()
        {
            string point = this.s_url3 + "/get_score_setting";
            ...
            try
            {
                ...
                var getDt = JsonSerializer.Deserialize<Round3_I_ScoreConfig_JSONData[]>(rbodyS);
                foreach(var dt in getDt.Where(z=>z.index==-810))
                {
                    return dt;
                }
                return new Round3_I_ScoreConfig_JSONData()
                {
                    index = -810,
                    ask_throw = 0,
                    correct = 1,
        ...
        public SendResult SendScoreConfigR3(Round3_I_ScoreConfig_JSONData data)
        {
            if (PluginHost.SendType == SendTypes.Sell)
            {
                {
                    string point = this.s_url3 + "/set_score_setting";
